...
public class BookingController {
    ...
    @GetMapping("create/{customerId}/{yardId}")
    public String showFormCreate(Model model,
                                 @PathVariable int customerId,
                                 @PathVariable int yardId) {
        BookingDto bookingDto = new BookingDto();
        bookingDto.setCustomer(customerService.findById(customerId));
...
//            redirectAttributes.addFlashAttribute("message", "Bạn đã chọn sân thành công! Thanh toán ngay để giữ sân nào!!!");
        ...
        return "redirect:/bookings/create/" + booking.getCustomer().getId() + "/" + booking.getYard().getId();
    }
